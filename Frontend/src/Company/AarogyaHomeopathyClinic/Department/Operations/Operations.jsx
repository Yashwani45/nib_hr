...
const Operations = ({ records, dbData }) => {
  // Filter employees belonging to this department
  const employees = dbData?.["Employee Profile"] || [];
  const deptEmployees = employees.filter(
    emp => emp.department?.toLowerCase() === "operations"
  );

  // Get department details from records if available
  const deptDetail = records?.[0] || {
    deptCode: "OPS-IND-004",
    deptName: "Operations",
    head: "Department Lead",
    description: "Handles billing, accounts, expenses, payroll coordination and financial records for the Indore branch.",
    status: "Active"
  };
