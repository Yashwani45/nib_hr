...
const QualityClinicalAudit = ({ records, dbData }) => {
  // Filter employees belonging to this department
  const employees = dbData?.["Employee Profile"] || [];
  const deptEmployees = employees.filter(
    emp => emp.department?.toLowerCase() === "quality & clinical audit"
  );

  // Get department details from records if available
  const deptDetail = records?.[0] || {
    deptCode: "QCA-IND-006",
    deptName: "Quality & Clinical Audit",
    head: "Department Lead",
    description: "Quality monitoring, clinic audits and compliance.",
    status: "Active"
  };
...
export default QualityClinicalAudit;
