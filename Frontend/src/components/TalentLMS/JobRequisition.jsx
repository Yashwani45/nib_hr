      ...
      {records.map((item) => (
        <div key={item.id} className="border border-gray-200/80 rounded-2xl p-5 bg-white shadow-sm flex flex-col justify-between space-y-3">
          <div className="flex justify-between items-center border-b pb-2">
            <span className="font-bold text-xs text-gray-800">Job Code: {item.reqId || item.jobCode || `#${item.id}`}</span>
            {item.status && (
              <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${item.status === "Active" || item.status === "Open" || item.status === "Approved" || item.status === "Completed" ? "bg-green-50 text-green-700 border-green-200" : "bg-amber-50 text-amber-700 border-amber-200"} border`}>
                {item.status}
              </span>
            )}
          </div>
